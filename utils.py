...
def sample_gaussian(m, v):
    """
    Aplicación elemento a elemento del truco de reparametrización para muestrear de una Gaussiana

    Args:
        m: tensor: (batch, ...): Media
        v: tensor: (batch, ...): Varianza
...
def log_normal(x, m, v):
    """
    Calcula la probabilidad logarítmica elemento a elemento de una Gaussiana y luego sumar sobre la
    última dimensión. Básicamente se asume que todas las dimensiones son de batch excepto la
    última dimensión.

    Args:
        x: tensor: (batch_1, batch_2, ..., batch_k, dim): Observación
        m: tensor: (batch_1, batch_2, ..., batch_k, dim): Media
        v: tensor: (batch_1, batch_2, ..., batch_k, dim): Varianza

    Return:
        log_prob: tensor: (batch_1, batch_2, ..., batch_k): probabilidad logarítmica de
            cada muestra. Tenga en cuenta que la dimensión de suma no se mantiene
    """
    ################################################################################
    ...
    # la última dimensión
    ################################################################################
     # log N(x | m, v) = -0.5 * [log(2πv) + (x-m)^2 / v]
    log_prob = -0.5 * (torch.log(2 * np.pi * v) + (x - m).pow(2) / v)

    # sumar sobre la última dimensión
...
def log_normal_mixture(z, m, v):
    """
    Calcula la probabilidad logarítmica de una mezcla de Gaussianas.

    Args:
        z: tensor: (batch, dim): Observaciones
        m: tensor: (batch, mix, dim): Medias de la mezcla
        v: tensor: (batch, mix, dim): Varianzas de la mezcla
    ...
    # El resultado tendrá una forma de (batch, mix).
    log_prob_componentes = log_normal(z_expandido, m, v)
...
def gaussian_parameters(h, dim=-1):
    """
    Convierte representaciones genéricas de valores reales en parámetros de media y varianza
    de una distribución Gaussiana

    Args:
        h: tensor: (batch, ..., dim, ...): Tensor arbitrario
        dim: int: (): Dimensión a lo largo de la cual dividir el tensor para media y
            varianza

    Returns:
        m: tensor: (batch, ..., dim / 2, ...): Media
        v: tensor: (batch, ..., dim / 2, ...): Varianza
    """
...
def log_bernoulli_with_logits(x, logits):
    """
    Calcula la probabilidad logarítmica de una Bernoulli dados sus logits

    Args:
        x: tensor: (batch, dim): Observación
        logits: tensor: (batch, dim): Logits de la Bernoulli
...
def kl_cat(q, log_q, log_p):
    """
    Calcula la divergencia KL entre dos distribuciones categóricas

    Args:
        q: tensor: (batch, dim): Parámetros de la distribución categórica
        log_q: tensor: (batch, dim): Logaritmo de q
        log_p: tensor: (batch, dim): Logaritmo de p

    Return:
        kl: tensor: (batch,) kl entre cada muestra
    """
...
def kl_normal(qm, qv, pm, pv):
    """
    Calcula la divergencia KL elemento a elemento entre dos distribuciones normales KL(q || p) y
    suma sobre la última dimensión. Para referencia: https://stats.stackexchange.com/questions/7440/kl-divergence-between-two-univariate-gaussians/7449#7449

    Args:
        qm: tensor: (batch, dim): media de q
        qv: tensor: (batch, dim): varianza de q
        pm: tensor: (batch, dim): media de p
        pv: tensor: (batch, dim): varianza de p
...
def duplicate(x, rep):
    """
    Duplica x a lo largo de la dimensión 0

    Args:
        x: tensor: (batch, ...): Tensor arbitrario
        rep: int: (): Número de réplicas. Si rep=1 retorna x original

    Returns:
        _: tensor: (batch * rep, ...): Tensor arbitrario replicado
    """
...
def log_mean_exp(x, dim):
    """
    Calcula log(mean(exp(x), dim)) de manera numéricamente estable

    Args:
        x: tensor: (...): Tensor arbitrario
        dim: int: (): Dimensión a lo largo de la cual se calcula la media
...
def log_sum_exp(x, dim=0):
    """
    Calcula log(sum(exp(x), dim)) de manera numéricamente estable

    Args:
        x: tensor: (...): Tensor arbitrario
        dim: int: (): Dimensión a lo largo de la cual se calcula la suma
...
def load_model_by_name(model, global_step, device=None):
    """
    Carga un modelo basado en su nombre model.name y el paso de iteración del checkpoint

    Args:
        model: Model: (): Un modelo
        global_step: int: (): Iteración del checkpoint
    """
